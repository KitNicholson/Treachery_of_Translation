...
let versionNum = '1.2.2 Sheet Music';
let fileName = 'Requiem Lacrimosa';
let filetype = '.mp3';
let artistName = 'Wolfgang Amadeus Mozart';
...
function setup() {
  ...
  lineSpacing = memorySpecHeight*0.3;
...
function addInfoForPrintBASE() {
  ...
  text(fileName, width*0.01, height-boxHeight*0.6);

  // draw the original artists name, and the code version as the arranger.
  textFont(authorReg);

  textSize(boxHeight*0.33);
  text('Composed by: ' + artistName + ". Interpreted by code", width*0.01, height-boxHeight*0.17);
}
...
function drawSoundFilePlayback_V() {
  ...
  if (keySoundFile.isPlaying()) {
    ...
    for (let i=0; i<spectrum.length; i++) {
      ...
      let hue = map(spectrum[i], 0, 255, 200, 50); // change these arguments to change the colour
      hue1 = hue % 360;

      let brightness = map(spectrum[i], 0,255, 100, 70);

      let alpha = map(spectrum[i], 0,255, 0, 0.5);
      ...
      if (i>0) {
        ...
        fill(hue3, 0, brightness3, alpha3);
        rect(width/2-quartX, mTime, w3, -h); // draw pixel
        rect(width/2+quartX, mTime, w3, -h); // draw pixel
    
        fill(hue4, 0, brightness4, alpha4);
        rect(width/2-threeQuartx, mTime, w4, -h); // draw pixel
...
function drawSoundFilePlayback_H() {
  ...
  if (keySoundFile.isPlaying()) {
    ...
    for (let i=0; i<spectrum.length; i++) {
      ...
      let brightness = map(spectrum[i], 0,255, 100, 0);

      let alpha = map(spectrum[i], 0,255, 0, 0.6);
...
function setMemoryTime(SoundFile) {
...
function drawMemoryTimeGaps() {
  ...
  if (recordingMemory) {
    ...
    for (let i=0; i<spectrum.length; i++) {
      ...
      hue1 = hue % 360;
      let brightness = map(spectrum[i], 0,255, 100, 0); // 0 is to match cavas background
    ...
    let noteWidth = memorySpecHeight/23; // 23 is good for a4 pages
